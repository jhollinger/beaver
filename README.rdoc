...
* Rails 3.2 tagged logging is cool, but what's a good way to review them? (Beaver is, since version 1.2.0)
...
=== DSL
In short, the innermost level uses AND. If there is an outer level, it uses OR. Passing more than two levels is not supported.
