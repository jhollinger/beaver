...
Read the full documentation at {jordanhollinger.com/docs/beaver/}[http://jordanhollinger.com/docs/beaver/]. The Beaver::Dam and Beaver::Request classes
...
== Use beaver like grep for Rails logs (DSL files are optional)

Pretty difficult to grep through a multi-line log format. But Beaver is up to it:

  beaver --path="/widgets.*" --method=post,put /var/www/rails-app/log/production.log

Or format the output:

  beaver --path="/widgets.*" --method=post,put --print "%{ip} hit %{path} using %{method}" /var/www/rails-app/log/production.log
