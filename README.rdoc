...
Write your beaver file:
  ...
  # Report on each widget that was created
  ...
  hit :broken_forms, :method => :post, :status => 404 do
    puts "Someone got a 404 posting to #{path}, passing #{params_str}"
  end
...
* Add support for parsing and matching based on response time
* Add support for parsing and matching based on request datetime
