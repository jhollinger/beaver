== Need to parse Rails log files? Get a Beaver to help!
...
Rails logs contain some great information, but I've never found a good tool for parsing them programatically (except for speed analyzers).
...
Run it, passing in some log files:
...
== DSLs are great and all, but what's really going on here?

  # This returns a new Beaver::Beaver object with some logs to parse
  beaver = Beaver.new('/var/www/rails-app/log/production.log*')

  # Parse the logs into Beaver::Request objects (available through beaver.requests)
  beaver.parse
...
== 3 Why Beaver?
Beavers chew through logs to make something useful.
